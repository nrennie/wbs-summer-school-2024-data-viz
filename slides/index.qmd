...
format:
  LUstyle-revealjs:
    footer: "Slides: [nrennie.rbind.io/wbs-summer-school-2024-data-viz](https://nrennie.rbind.io/wbs-summer-school-2024-data-viz/)"
    self-contained: true
    auto-stretch: false
execute: 
  eval: true
...
* In groups, discuss how you might visualise the `scurvy` data.

* Use pen and paper, R, or Python. 
...
data("scurvy")
ggplot(scurvy) +
  geom_col(aes(x = study_id, y = gum_rot_d6))
```
...
scurvy |>
  tidyr::pivot_longer(
    gum_rot_d6:lassitude_d6,
    names_to = "symptom"
  ) |>
  ggplot() +
  geom_col(aes(x = study_id, y = value,
               fill = treatment)) +
  facet_wrap(~symptom) +
...
* Edit your plot of the `scurvy` data from Discussion 1 to make better use of colour, text, and styling.
* Alternatively, edit the code for example plots on the next slides.
...
# sort symptoms
scurvy["gum_rot_d6"] = pd.Categorical(scurvy["gum_rot_d6"], ["0_none", "1_mild", "2_moderate", "3_severe"], ordered = True)
scurvy = scurvy.sort_values('gum_rot_d6')

# plot
plt.bar(scurvy['study_id'], scurvy['gum_rot_d6'], alpha=0.7, color='skyblue')
...
{{< fa brands twitter >}} [@nrennie35](https://x.com/nrennie35)
